...
public sealed class OpenAiPassthroughTests : IClassFixture<ProxyTestFactory>
{
    ...
    [Fact]
    public async Task AdaptiveApi_extension_headers_are_stripped_before_upstream()
    {
        ...
        var captured = Assert.Single(_factory.Upstream.Requests);
        Assert.False(captured.HasHeader("X-AdaptiveApi-Target-Lang"),
            "adaptiveapi extension header leaked to upstream");
