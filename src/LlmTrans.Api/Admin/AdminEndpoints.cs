...
public static class AdminEndpoints
{
    ...
    private static async Task<IResult> CreateTenant(CreateTenantDto dto, LlmTransDbContext db, CancellationToken ct)
    {
    ...
    private static async Task<IResult> ListTenants(LlmTransDbContext db, CancellationToken ct)
    {
    ...
    private static async Task<IResult> CreateRoute(CreateRouteDto dto, LlmTransDbContext db, CancellationToken ct)
    {
    ...
    private static async Task<IResult> ListRoutes(LlmTransDbContext db, CancellationToken ct)
    {
        var routes = await db.Routes.AsNoTracking().ToListAsync(ct);
        return Results.Ok(routes.Select(ToDto));
    }

    private static async Task<IResult> UpdateRoute(string id, UpdateRouteDto dto, LlmTransDbContext db, CancellationToken ct)
    {
    ...
    private static async Task<IResult> DeleteRoute(string id, LlmTransDbContext db, CancellationToken ct)
    {
        var r = await db.Routes.FirstOrDefaultAsync(x => x.Id == id, ct);
        if (r is null) return Results.NotFound();
        db.Routes.Remove(r);
    ...
    private static async Task<IResult> IssueToken(string id, LlmTransDbContext db, CancellationToken ct)
    {
        var r = await db.Routes.FirstOrDefaultAsync(x => x.Id == id, ct);
        if (r is null) return Results.NotFound();
    ...
    private static async Task<IResult> ListTokens(string id, LlmTransDbContext db, CancellationToken ct)
    {
    ...
    private static async Task<IResult> RevokeToken(string tokenId, LlmTransDbContext db, CancellationToken ct)
    {
