...
public sealed class AuthOptions
{
    ...
    public string CookieName { get; set; } = "llmtrans.session";
    public string[] Scopes { get; set; } = new[] { "openid", "profile", "email" };
}

public static class AuthSetup
{
    public const string AdminPolicy = "llmtrans-admin";
    public const string CookieScheme = "llmtrans-cookie";

    public static AuthMode Configure(WebApplicationBuilder builder)
    {
        var opts = new AuthOptions();
        builder.Configuration.GetSection("LlmTrans:Auth").Bind(opts);
        ...
        if (string.IsNullOrEmpty(opts.Authority))
            throw new InvalidOperationException("LlmTrans:Auth:Mode=oidc requires LlmTrans:Auth:Authority");
