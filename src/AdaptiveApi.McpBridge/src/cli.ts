...
async function main(argv: readonly string[]): Promise<number> {
  ...
  try {
    ...
    if (err instanceof ArgsError) {
      process.stderr.write(`adaptiveapi-mcp-bridge: ${err.message}\n\n${helpText}\n`);
      return 2;
    }
    throw err;
  }
...
main(process.argv.slice(2)).then(
  ...
  (err) => {
    process.stderr.write(`adaptiveapi-mcp-bridge fatal: ${err instanceof Error ? err.stack ?? err.message : String(err)}\n`);
    process.exit(70);
  },
);
