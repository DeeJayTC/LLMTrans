...
Local stdio ↔ llmtrans translate-API bridge for MCP clients (Claude Desktop, Cursor,
Zed, VS Code + Continue, …).

## What it does

Your MCP client already knows how to spawn the stdio MCP server it wants and set the
right env vars. `@llmtrans/mcp-bridge` sits between the two, translating every
...
llmtrans for translation.

## Install

Usually you don't install it; MCP clients run `npx @llmtrans/mcp-bridge …` on demand.
...
### After (bridged through llmtrans)
...
Requests" in German — the bridge sends your question to llmtrans, llmtrans
...
| --- | --- | --- |
| `--route` / `-r` | *required* | Your llmtrans route token. |
| `--endpoint` / `-e` | `https://api.llmtrans.example.com` | Base URL of llmtrans. |
...
Before committing your client config, verify the bridge can reach llmtrans and
that your route token resolves:

```bash
npx @llmtrans/mcp-bridge doctor --route rt_your_token --endpoint https://api.llmtrans.example.com
```

Sample output:

```
llmtrans-mcp-bridge doctor: PASS
...
If llmtrans is temporarily unreachable or returns a non-2xx response, the bridge
**forwards the untranslated message** and logs a one-line warning to stderr. Your
MCP server keeps working; you just won't get translation until llmtrans is back.
