...
const DEFAULT_ENDPOINT = 'https://api.llmtrans.example.com';
...
export function parseArgs(argv: readonly string[]): BridgeArgs {
  ...
  if (!args.route) {
    throw new ArgsError('missing --route (your llmtrans route token)');
  }
...
@llmtrans/mcp-bridge — local stdio ↔ llmtrans translate-API bridge
...
FLAGS
  ...
  --endpoint, -e    Base URL of the llmtrans API. Default: ${DEFAULT_ENDPOINT}
...
EXAMPLE
  ...
  Wrap it with llmtrans-mcp-bridge — env is inherited verbatim, no change:
    {
      "mcpServers": {
        "github-de": {
          "command": "npx",
  ...
  as a child and only JSON-RPC message bodies pass through llmtrans for translation.
`;
