...
/// translating each JSON-RPC message in flight via llmtrans's stateless translate API.
///
...
///   2. Parse. If parsing fails, forward verbatim — llmtrans's contract is JSON-RPC only.
...
export async function runBridge(args: BridgeArgs, io: BridgeIo): Promise<number> {
  ...
  upstream.on('error', (err) => {
    io.stderr.write(`[llmtrans-mcp-bridge] failed to spawn upstream: ${err.message}\n`);
  });
...
function pumpStdio(
  ...
  input.on('error', (err) => {
    io.stderr.write(`[llmtrans-mcp-bridge] ${direction} read error: ${err.message}\n`);
  });
}
