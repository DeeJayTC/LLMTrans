  ...
  async translate(direction: Direction, message: JsonRpcMessage): Promise<JsonRpcMessage> {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        // even if llmtrans is temporarily unavailable.
        const body = await resp.text().catch(() => '');
        this.stderr.write(
          `[llmtrans-mcp-bridge] translate ${resp.status} — forwarding untranslated (${body.slice(0, 120)})\n`,
        );
        return message;
      }
      ...
      this.stderr.write(
        `[llmtrans-mcp-bridge] translate failed — forwarding untranslated: ${reason}\n`,
      );
      return message;
    } finally {
