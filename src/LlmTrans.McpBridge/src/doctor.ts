...
/// can reach llmtrans and that the route token resolves. Emits a structured result;
...
export function formatDoctorResult(r: DoctorResult): string {
  const lines: string[] = [];
  lines.push(r.ok ? 'llmtrans-mcp-bridge doctor: PASS' : 'llmtrans-mcp-bridge doctor: FAIL');
