...
public static class McpEndpoints
{
    ...
    private static async Task<IResult> CreateServer(CreateServerDto dto, AdaptiveApiDbContext db, CancellationToken ct)
    {
    ...
    private static async Task<IResult> ListServers(AdaptiveApiDbContext db, CancellationToken ct) =>
        Results.Ok(await db.McpServers.AsNoTracking().Select(ToSelectDto).ToListAsync(ct));

    private static async Task<IResult> GetServer(string id, AdaptiveApiDbContext db, CancellationToken ct)
    {
    ...
    private static async Task<IResult> DeleteServer(string id, AdaptiveApiDbContext db, CancellationToken ct)
    {
    ...
    private static async Task<IResult> GetSnippet(string id, string? client, AdaptiveApiDbContext db, HttpContext ctx, CancellationToken ct)
    {
        var s = await db.McpServers.AsNoTracking().FirstOrDefaultAsync(x => x.Id == id, ct);
        if (s is null) return Results.NotFound();
    ...
    private static async Task<IResult> ListCatalog(AdaptiveApiDbContext db, CancellationToken ct) =>
    ...
    private static async Task<IResult> GetCatalogEntry(string slug, AdaptiveApiDbContext db, CancellationToken ct)
    {
