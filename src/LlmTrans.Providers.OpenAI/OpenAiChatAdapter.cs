...
namespace LlmTrans.Providers.OpenAI;
...
public sealed class OpenAiChatAdapter : IProviderAdapter
{
    ...
    {
        var userLang = req.Headers.TryGetValue("X-LlmTrans-Target-Lang", out var tl)
            ? new LanguageCode(tl.ToString().Trim().ToLowerInvariant())
            : route.UserLanguage;
        var llmLang = req.Headers.TryGetValue("X-LlmTrans-Source-Lang", out var sl)
            ? new LanguageCode(sl.ToString().Trim().ToLowerInvariant())
            : route.LlmLanguage;
        var mode = req.Headers.TryGetValue("X-LlmTrans-Mode", out var m)
            ? Enum.TryParse<DirectionMode>(m.ToString(), true, out var parsed) ? parsed : route.Direction
            : route.Direction;
        var translator = req.Headers.TryGetValue("X-LlmTrans-Translator", out var t)
            ? t.ToString() : route.TranslatorId;
        var glossary = req.Headers.TryGetValue("X-LlmTrans-Glossary", out var g) ? g.ToString() : route.GlossaryId;
        // Header precedence:
        //   X-LlmTrans-Request-Style-Rule / X-LlmTrans-Response-Style-Rule override per direction.
        //   X-LlmTrans-Style-Rule (legacy) overrides both directions unless a direction-specific
        //   header is also present.
        string? legacyStyle = req.Headers.TryGetValue("X-LlmTrans-Style-Rule", out var srv)
            ? srv.ToString() : null;
        var requestStyle = req.Headers.TryGetValue("X-LlmTrans-Request-Style-Rule", out var rsr)
            ? rsr.ToString()
            : legacyStyle ?? route.RequestStyleRuleId;
        var responseStyle = req.Headers.TryGetValue("X-LlmTrans-Response-Style-Rule", out var rspsr)
            ? rspsr.ToString()
    ...
    /// Picks a streaming strategy. Request header `X-LlmTrans-Stream-Strategy: progressive`
    ...
    {
        if (req.Headers.TryGetValue("X-LlmTrans-Stream-Strategy", out var v))
    ...
    {
        if (!req.Headers.TryGetValue("X-LlmTrans-Debug", out var v)) return false;
    ...
    /// `x-llmtrans-timing` SSE event carries the same information Server-Timing
