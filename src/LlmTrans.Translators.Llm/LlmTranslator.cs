...
public sealed class LlmTranslator : ITranslator
{
    ...
    private readonly IOptions<LlmTranslatorOptions> _options;
    private readonly ILogger<LlmTranslator> _log;

    public LlmTranslator(IHttpClientFactory httpFactory, IOptions<LlmTranslatorOptions> options, ILogger<LlmTranslator> log)
    {
    ...
    private static List<Placeholder> ExtractPlaceholders(string source)
    {
        ...
        while (true)
        {
            var start = source.IndexOf("<llmtrans id=\"", i, StringComparison.Ordinal);
            if (start < 0) break;
            var idStart = start + "<llmtrans id=\"".Length;
    ...
    private static string BuildSystemPrompt(TranslationRequest r)
    {
        var instructions = new List<string>
        {
            ...
            "Preserve every <llmtrans id=\"TAG_n\"/> tag exactly — do not translate, reorder, or remove them.",
