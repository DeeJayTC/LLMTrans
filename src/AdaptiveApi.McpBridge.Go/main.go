...
//
// Behavior parity with the Node CLI at ../AdaptiveApi.McpBridge, minus the npm-install
...
//
// Usage:
//     adaptiveapi-mcp-bridge --route rt_xxx [--endpoint URL] [--passthrough] [--log PATH] -- <cmd> [args...]
//     adaptiveapi-mcp-bridge doctor --route rt_xxx [--endpoint URL]
package main
...
const (
	defaultEndpoint = "https://api.adaptiveapi.example.com"
...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "adaptiveapi-mcp-bridge: %v\n\n%s\n", err, helpText)
		os.Exit(2)
	}
	...
	case "bridge":
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "adaptiveapi-mcp-bridge fatal: %v\n", err)
			os.Exit(70)
		}
		os.Exit(code)
	}
...
func parseArgs(argv []string) (*args, error) {
	...
	if a.route == "" {
		return nil, errors.New("missing --route (your adaptiveapi route token)")
	}
...
func pump(in io.Reader, out io.Writer, direction string, a *args, client *http.Client, logFile io.Writer) {
	...
	for scanner.Scan() {
		...
		if !a.passthrough {
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "[adaptiveapi-mcp-bridge] translate failed — forwarding untranslated: %v\n", err)
	...
	if err := scanner.Err(); err != nil && err != io.EOF {
		fmt.Fprintf(os.Stderr, "[adaptiveapi-mcp-bridge] %s read error: %v\n", direction, err)
	}
}
...
func doctor(a *args) error {
	...
	if ok {
		fmt.Println("adaptiveapi-mcp-bridge doctor:")
	}
...
const helpText = `adaptiveapi-mcp-bridge — local stdio ↔ adaptiveapi translate-API bridge (Go)
...
FLAGS
  ...
  --endpoint, -e    Base URL of the adaptiveapi API. Default: https://api.adaptiveapi.example.com
...
See the Node bridge README at ../AdaptiveApi.McpBridge/README.md for a full walkthrough.
`
